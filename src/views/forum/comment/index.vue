<template>
  <div class="app-container">
    <el-card>
      ...
      >
        ...
        <el-table-column prop="title" label="回复文章"> </el-table-column>
        <el-table-column prop="pContent" label="回复评论"> </el-table-column>
        <el-table-column prop="replyUsername" label="回复人"> </el-table-column>
        ...
        <el-table-column prop="createTime" label="评论时间" width="240px"> </el-table-column>
  ...
  <script>
import { getComments, delArticleComment, delCommentByBatch } from "@/api/forum";
export default {
  methods: {
    async deletedBatch() {
      ...
      if (confirmResult != "confirm") {
        ...
        delCommentByBatch(this.selectList).then((resp) => {
          if (resp.code != 200) {
    ...
    async remove(id) {
      ...
      if (confirmResult != "confirm") {
        ...
        delArticleComment(id).then((resp) => {
          if (resp.code != 200) {
            return this.$message.error("删除失败！");
          } else {
            this.$message.success("删除成功！");
    ...
    async fetchData() {
      ...
      getComments(this.queryInfo).then((response) => {
